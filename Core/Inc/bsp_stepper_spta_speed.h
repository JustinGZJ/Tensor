...
/* SPTA相关参数结构体 */
typedef struct {
    __IO int32_t steps_required;       //总步数
    __IO uint32_t steps_taken;         //已走过的步数

    __IO uint32_t steps_acced;         //加速阶段的步数
    __IO uint32_t steps_middle;        //总步数的中点，三角轮廓时超过该步数就要减速

    __IO uint32_t acceleration;        //加速度的数值，不包括方向
    __IO uint32_t VelAccumulator;   //速度累加器，每次累加step_accel，当累加器第17位置位后step_speed增加


    __IO uint32_t ActualVelocity;          //SPTA速度值，speed_accumulator的溢出值加到这里
    __IO uint32_t step_accumulator;    //步数累加器，每次累加step_speed，当累加器第17位置位后产生一个步进脉冲
    __IO uint32_t half_step_accumulator;

    __IO uint32_t TargetVelocity;           //电机的最大速度限制，运行速度小于速度限制时为三角轮廓，大于等于速度限制时为梯形轮廓
    STEP_MODE_TypeDef step_mode;
